...
public class PersonImageRepositoryIntegrationTest extends AbstractRepositoryIDIntegrationTest<PersonImage> {

	public PersonImageRepositoryIntegrationTest() {
	...
	protected AbstractRepository<PersonImage> getRepository() {
		return new PersonImageRepository();
	}
	...
	public void testFindByPerson()
	{
		...
		manager.persist(img);
		manager.flush();
		PersonImage otherImage = ((PersonImageRepository)repository).getByPersonId(img.getPerson().getId());
