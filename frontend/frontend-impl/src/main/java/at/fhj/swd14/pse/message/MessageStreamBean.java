...
public class MessageStreamBean implements Serializable
{
	...
	*/
	...
	public void onCommunityChange()
	{
		...
		if (this.currentCommunity.getId() <= -3)
		{ // All Messages
			this.messages = this.getAllMessages();
		} else if (this.currentCommunity.getId() == -2)
		{ // Private Messages
			this.messages = this.getPrivateMessages();
		} else if (this.currentCommunity.getId() == -1)
		{ // Global Messages
			this.messages = this.getGlobalMessages();
		} else if (this.currentCommunity.getId() == 0)
		{ // All Messages
			this.messages = this.getAllMessages();
		} else if (this.currentCommunity.getId() > 0)
		{ // Specific Community Message
			this.messages = this.getCommunityMessages(this.currentCommunity.getId());
		} else
		{
			this.messages = this.getAllMessages(); // Default -> here should be
													// some ErrorHandling
		}
	}
